...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template name="SAAge">
        <xsl:param name="item"/>
        <xsl:variable name="ageNum" select="substring-before($item/age,' ')"></xsl:variable>
        <xsl:variable name="ageTime" select="substring-after($item/age,' ')"></xsl:variable>
        <xsl:value-of select="$ageNum"/>
        <xsl:call-template name="AgeTime">
            <xsl:with-param name="ageTime" select="$ageTime"/>
        </xsl:call-template>
     </xsl:template>
